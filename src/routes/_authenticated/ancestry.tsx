...
function generationLabel(gen: number, horizontal: boolean): string {
  if (gen === 0) return horizontal ? "Frate / soră" : "Frate / soră";
  if (gen > 0) {
    if (gen === 1) return "Părinte";
    if (gen === 2) return "Bunic";
    return `${"Mare-".repeat(gen - 2)}bunic`;
  }
  const d = -gen;
  if (d === 1) return "Copil";
  if (d === 2) return "Nepot";
  return `${"Mare-".repeat(d - 2)}nepot`;
}
...
function AncestryInner() {
    ...
    (async () => {
      ...
      if (data) {
        ...
        const seed: Node = {
          ...
          data: { name: "Tu", relation: "Tu (rădăcină)", isRoot: true } satisfies PersonData,
        };
        setNodes([seed]);
        setRootId(seedId);
      }
  ...
  const addPerson = () => {
    ...
    const name = window.prompt("Numele persoanei?");
    if (!name) return;
    const imageUrl = window.prompt("URL imagine? (opțional)") || undefined;
    const newNode: Node = {
      id, type: "person",
      position: { x: 100 + Math.random() * 300, y: 100 + Math.random() * 300 },
      data: { name, imageUrl, relation: "Netrimis" } satisfies PersonData,
    };
    setNodes((nds) => applyRelations([...nds, newNode], edges, rootId));
  };

  const makeRoot = () => {
    if (!selected) return toast.info("Selectează mai întâi un nod");
    setRootId(selected);
    setNodes((nds) => applyRelations(nds, edges, selected));
    toast.success("Rădăcina a fost actualizată");
  };

  const deleteSelected = () => {
    if (!selected) return;
    if (selected === rootId) return toast.error("Nu poți șterge rădăcina");
  ...
  const onEdgeDoubleClick = useCallback((_: MouseEvent, edge: Edge) => {
    const label = window.prompt("Etichetă muchie?", (edge.label as string) ?? "");
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success("Arborele a fost salvat");
    } catch (error: any) {
      toast.error(error?.message ?? "Nu s-a putut salva arborele");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <div className="flex h-[calc(100vh-3.5rem)] items-center justify-center text-sm text-muted-foreground">Se încarcă arborele…</div>;
  }
    ...
    <div className="relative h-[calc(100vh-3.5rem)] w-full">
      ...
      <div className="absolute left-4 top-4 z-10 flex flex-wrap items-center gap-2 rounded-lg border bg-card/95 p-2 shadow-md backdrop-blur">
        <button onClick={addPerson} className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">
          <Plus className="h-4 w-4" /> Adaugă persoană
        </button>
        <button onClick={makeRoot} disabled={!selected} className="inline-flex items-center gap-1.5 rounded-md border bg-background px-3 py-1.5 text-sm font-medium hover:bg-accent disabled:opacity-50">
          <Crown className="h-4 w-4" /> Setează ca rădăcină
        </button>
      ...
      <div className="absolute right-4 top-4 z-10 w-64 rounded-lg border bg-card/95 p-3 text-xs text-muted-foreground shadow-md backdrop-blur">
        {selectedNode ? (
          <>
            ...
            <div className="mt-1">Relație: {(selectedNode.data as PersonData).relation}</div>
          </>
        ) : (
          <div className="text-sm font-semibold text-foreground">Sfaturi</div>
        )}
        <ul className="mt-2 list-disc space-y-1 pl-4">
          <li>Trage vertical pentru a schimba generația.</li>
          <li>Conectează din orice parte — orizontal = frate/soră.</li>
          <li>Duble-clic pe o muchie pentru a-i pune etichetă.</li>
          <li>Selectează o muchie și apasă Delete pentru a o elimina.</li>
          <li>Relația = distanța verticală + conexiunea prin muchii.</li>
        </ul>
      </div>
    </div>
  );
